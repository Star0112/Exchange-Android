...
public class OtpView extends AppCompatEditText {
    ...
    private boolean showLineWhenActive, hideLineWhenFilled;
    ...
    public OtpView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        showLineWhenActive = typedArray.getBoolean(R.styleable.OtpView_showLineWhenActive, false);
    ...
    private void drawInput(Canvas canvas, int i) {
        if (isPasswordInputType(getInputType()) && getText() != null) {
            if (!rtlTextDirection && i < getText().length() - 1 || rtlTextDirection && i > otpViewItemCount - getText().length()) {
                drawCircle(canvas, i);
            } else {
                drawText(canvas, i);
            }
    ...
    private void drawOtpBox(Canvas canvas, int i) {
        if (getText() != null && (showLineWhenActive && i != getText().length() || hideLineWhenFilled && i < getText().length())) {
            return;
        }
    ...
    private void drawCircle(Canvas canvas, int i) {
        ...
        float cy = itemCenterPoint.y;
        if (rtlTextDirection) {
            if (false) { // TODO: why is this logic?
                int reversedItemPosition = otpViewItemCount - i;
                int reversedCharPosition = reversedItemPosition - (getHint() == null ? 0 : getHint().length());
                if (reversedCharPosition <= 0) {
                    canvas.drawCircle(cx, cy, paint.getTextSize() / 3, paint);
                }
            } else {
                canvas.drawCircle(cx, cy, paint.getTextSize() / 3, paint);
            }
     ...
     *
     * @param showLineWhenActive true to show line on a current position,
     * false to always show line
     * @attr ref R.styleable#OtpView_showLineWhenActive
     */
    public void setShowLineWhenActive(boolean showLineWhenActive) {
        this.showLineWhenActive = showLineWhenActive;
    }
