...
public class MainActivity extends BaseActivity implements ApiCallback {
    ...
    private final DashboardFragment fragment1 = new DashboardFragment();
//    private final CardContainerFragment fragment2 = new CardContainerFragment_();
//    private final GiftContainerFragment fragment3 = new GiftContainerFragment_();
    private final ExchangeFragment fragment4 = new ExchangeFragment_();
//    private final ProfileContainerFragment fragment5 = new ProfileContainerFragment_();
    private final FragmentManager fm = getSupportFragmentManager();
    private BaseFragment active = fragment4;
    ...
    private BottomNavigationView.OnNavigationItemSelectedListener mOnNavigationItemSelectedListener = item -> {
        switch (item.getItemId()) {
            ...
            case R.id.navigation_card:
                if (AppData.getInstance().getWallets().size() == 0 || AppData.getInstance().getExchangeTickers().size() == 0) {
                    showAlert(R.string.no_wallets_to_exchange);
                    return false;
                }
                fm.beginTransaction().hide(active).show(fragment4).commitAllowingStateLoss();
                active = fragment4;
                break;
//                if (fragment2.activeFragment == null) return false;
//                if (false && fragment2.activeFragment instanceof CardOrderFragment) {
//                    for (CardInfo card : AppData.getInstance().getAvailableCards()) {
//                        if (!card.isGift()) { // available card exists for this user
...
//                        }
//                    }
//                    showAlert(R.string.error_available_cards);
//                    return false;
//                } else {
//                    fm.beginTransaction().hide(active).show(fragment2).commitAllowingStateLoss();
//                    active = fragment2;
...
//        fm.beginTransaction().add(R.id.container, fragment4, "5").hide(fragment4).commit();
//        fm.beginTransaction().add(R.id.container, fragment4, "3").hide(fragment4).commit();
//        fm.beginTransaction().add(R.id.container, fragment4, "2").hide(fragment4).commit();
//        fm.beginTransaction().add(R.id.container, fragment4, "1").hide(fragment4).commit();
...
//        fm.beginTransaction().add(R.id.container, fragment2, "2").hide(fragment2).commit();
        ...
        setActiveTab(R.id.navigation_wallet, null);
...
//        getExchangeTickersData();
    ...
    private void updateTabIcons(int itemId) { // not recommended but because of bad icon designs. it should use opacity instead of color
        navigation.getMenu().findItem(R.id.navigation_wallet).setIcon(itemId == R.id.navigation_wallet ? R.mipmap.ic_tab_wallet_selected : R.mipmap.ic_tab_wallet);
        navigation.getMenu().findItem(R.id.navigation_card).setIcon(itemId == R.id.navigation_card ? R.mipmap.ic_tab_card_selected : R.mipmap.ic_tab_card);
        ...
        navigation.getMenu().findItem(R.id.navigation_exchange).setIcon(itemId == R.id.navigation_exchange ? R.mipmap.ic_tab_exchange_selected : R.mipmap.ic_tab_exchange);
...
//                fragment2.getData(verifyType == Constants.VerifyType.CARD_ACTIVATE); // refresh my cards info
    ...
    public void setActiveTab(int navigationId, Wallet wallet) {
        ...
        if (wallet != null) {
            if (navigationId == R.id.navigation_wallet) {
...
//        }
    ...
    public void onBackPressed() {
        ...
        } else if (navigation.getSelectedItemId() == R.id.navigation_wallet) {
