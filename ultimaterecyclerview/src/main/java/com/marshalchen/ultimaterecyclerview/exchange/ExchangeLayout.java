...
public class ExchangeLayout extends FrameLayout {
    @Deprecated
    ...
    private float[] mEdgeExchangesOffset = new float[4];

    private List<ExchangeListener> mExchangeListeners = new ArrayList<>();
    private List<ExchangeDenier> mExchangeDeniers = new ArrayList<>();
    ...
    private boolean mExchangeEnabled = true;
    private boolean[] mExchangesEnabled = new boolean[]{true, true, true, true};
    ...
    public ExchangeLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        mDragHelper = ViewDragHelper.create(this, mDragHelperCallback);
        mTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop();

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ExchangeLayout);
        int dragEdgeChoices = a.getInt(R.styleable.ExchangeLayout_drag_edge, DRAG_RIGHT);
        mEdgeExchangesOffset[DragEdge.Left.ordinal()] = a.getDimension(R.styleable.ExchangeLayout_leftEdgeSwipeOffset, 0);
        mEdgeExchangesOffset[DragEdge.Right.ordinal()] = a.getDimension(R.styleable.ExchangeLayout_rightEdgeSwipeOffset, 0);
        mEdgeExchangesOffset[DragEdge.Top.ordinal()] = a.getDimension(R.styleable.ExchangeLayout_topEdgeSwipeOffset, 0);
        mEdgeExchangesOffset[DragEdge.Bottom.ordinal()] = a.getDimension(R.styleable.ExchangeLayout_bottomEdgeSwipeOffset, 0);
        setClickToClose(a.getBoolean(R.styleable.ExchangeLayout_clickToClose, mClickToClose));
        ...
        int ordinal = a.getInt(R.styleable.ExchangeLayout_show_mode, ShowMode.PullOut.ordinal());
    ...
    public interface ExchangeListener {
        void onStartOpen(ExchangeLayout layout);

        void onOpen(ExchangeLayout layout);

        void onStartClose(ExchangeLayout layout);

        void onClose(ExchangeLayout layout);

        void onUpdate(ExchangeLayout layout, int leftOffset, int topOffset);

        void onHandRelease(ExchangeLayout layout, float xvel, float yvel);
    }

    public void addExchangeListener(ExchangeListener l) {
        mExchangeListeners.add(l);
    }

    public void removeExchangeListener(ExchangeListener l) {
        mExchangeListeners.remove(l);
    }

    public void removeAllExchangeListener() {
        mExchangeListeners.clear();
    }

    public interface ExchangeDenier {
        /*
         * Called in onInterceptTouchEvent Determines if this exchange event should
         * be denied Implement this interface if you are using views with exchange
         * gestures As a child of ExchangeLayout
         *
         * @return true deny false allow
         */
        boolean shouldDenyExchange(MotionEvent ev);
    }

    public void addExchangeDenier(ExchangeDenier denier) {
        mExchangeDeniers.add(denier);
    }

    public void removeExchangeDenier(ExchangeDenier denier) {
        mExchangeDeniers.remove(denier);
    }

    public void removeAllExchangeDeniers() {
        mExchangeDeniers.clear();
    }
     ...
     * {@link com.marshalchen.ultimaterecyclerview.exchange.ExchangeLayout.OnRevealListener}
     *
    ...
    public void addRevealListener(int childId, OnRevealListener l) {
        ...
        if (child == null) {
            throw new IllegalArgumentException("Child does not belong to ExchangeListener.");
        }
     ...
     * @param l        the {@link com.marshalchen.ultimaterecyclerview.exchange.ExchangeLayout.OnRevealListener}
     */
    ...
    private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            for (ExchangeListener l : mExchangeListeners) {
                l.onHandRelease(ExchangeLayout.this, xvel, yvel);
            }
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            dispatchExchangeEvent(evLeft, evTop, dx, dy);
    ...
    protected void dispatchExchangeEvent(int surfaceLeft, int surfaceTop, int dx, int dy) {
        ...
        dispatchExchangeEvent(surfaceLeft, surfaceTop, open);
    }

    protected void dispatchExchangeEvent(int surfaceLeft, int surfaceTop, boolean open) {
        safeBottomView();
        Status status = getOpenStatus();

        if (!mExchangeListeners.isEmpty()) {
            mEventCounter++;
            for (ExchangeListener l : mExchangeListeners) {
                if (mEventCounter == 1) {
                    if (open) {
                ...
                l.onUpdate(ExchangeLayout.this, surfaceLeft - getPaddingLeft(), surfaceTop - getPaddingTop());
            }

            if (status == Status.Close) {
                for (ExchangeListener l : mExchangeListeners) {
                    l.onClose(ExchangeLayout.this);
                }
                mEventCounter = 0;
            }
    ...
    public interface OnLayout {
        void onLayout(ExchangeLayout v);
    }
    ...
    private void checkCanDrag(MotionEvent ev) {
        ...
        if (getOpenStatus() == Status.Close) {
            DragEdge dragEdge;
            if (angle < 45) {
                if (distanceX > 0 && isLeftExchangeEnabled()) {
                    dragEdge = DragEdge.Left;
                } else if (distanceX < 0 && isRightExchangeEnabled()) {
                    dragEdge = DragEdge.Right;
                } else return;

            } else {
                if (distanceY > 0 && isTopExchangeEnabled()) {
                    dragEdge = DragEdge.Top;
                } else if (distanceY < 0 && isBottomExchangeEnabled()) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        for (ExchangeDenier denier : mExchangeDeniers) {
            if (denier != null && denier.shouldDenyExchange(ev)) {
                return false;
            }
        }
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                sX = ev.getRawX();
                sY = ev.getRawY();
                //if the exchange is in middle state(scrolling), should intercept the touch
                if (getOpenStatus() == Status.Middle) {
                    mIsBeingDragged = true;
                }
                break;
            case MotionEvent.ACTION_MOVE:
                ...
                if (!beforeCheck && mIsBeingDragged) {
                    //let children has one chance to catch the touch, and request the exchange not intercept
                    //useful when exchangeLayout wrap a exchangeLayout or other gestural layout
                    return false;
                }
                break;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (!isExchangeEnabled()) return super.onTouchEvent(event);
    ...
    public void setExchangeEnabled(boolean enabled) {
        mExchangeEnabled = enabled;
    }

    public boolean isExchangeEnabled() {
        return mExchangeEnabled;
    }

    public boolean isLeftExchangeEnabled() {
        View bottomView = mDragEdges.get(DragEdge.Left);
        return bottomView != null && bottomView.getParent() == this
                && bottomView != getSurfaceView() && mExchangesEnabled[DragEdge.Left.ordinal()];
    }

    public void setLeftExchangeEnabled(boolean leftExchangeEnabled) {
        this.mExchangesEnabled[DragEdge.Left.ordinal()] = leftExchangeEnabled;
    }

    public boolean isRightExchangeEnabled() {
        View bottomView = mDragEdges.get(DragEdge.Right);
        return bottomView != null && bottomView.getParent() == this
                && bottomView != getSurfaceView() && mExchangesEnabled[DragEdge.Right.ordinal()];
    }

    public void setRightExchangeEnabled(boolean rightExchangeEnabled) {
        this.mExchangesEnabled[DragEdge.Right.ordinal()] = rightExchangeEnabled;
    }

    public boolean isTopExchangeEnabled() {
        View bottomView = mDragEdges.get(DragEdge.Top);
        return bottomView != null && bottomView.getParent() == this
                && bottomView != getSurfaceView() && mExchangesEnabled[DragEdge.Top.ordinal()];
    }

    public void setTopExchangeEnabled(boolean topExchangeEnabled) {
        this.mExchangesEnabled[DragEdge.Top.ordinal()] = topExchangeEnabled;
    }

    public boolean isBottomExchangeEnabled() {
        View bottomView = mDragEdges.get(DragEdge.Bottom);
        return bottomView != null && bottomView.getParent() == this
                && bottomView != getSurfaceView() && mExchangesEnabled[DragEdge.Bottom.ordinal()];
    }

    public void setBottomExchangeEnabled(boolean bottomExchangeEnabled) {
        this.mExchangesEnabled[DragEdge.Bottom.ordinal()] = bottomExchangeEnabled;
    }
    ...
    private void performAdapterViewItemClick() {
        ...
        if (t instanceof AdapterView) {
            ...
            int p = view.getPositionForView(ExchangeLayout.this);
    ...
    private boolean performAdapterViewItemLongClick() {
        ...
        if (t instanceof AdapterView) {
            ...
            try {
                ...
                handled = (boolean) m.invoke(view, ExchangeLayout.this, p, vId);

            } catch (Exception e) {
                e.printStackTrace();

                if (view.getOnItemLongClickListener() != null) {
                    handled = view.getOnItemLongClickListener().onItemLongClick(view, ExchangeLayout.this, p, vId);
                }
                if (handled) {
    ...
    private GestureDetector gestureDetector = new GestureDetector(getContext(), new ExchangeDetector());

    class ExchangeDetector extends GestureDetector.SimpleOnGestureListener {
        @Override
        ...
        public boolean onDoubleTap(MotionEvent e) {
            if (mDoubleClickListener != null) {
                ...
                mDoubleClickListener.onDoubleClick(ExchangeLayout.this, target == surface);
            }
            return true;
        }
    }
     ...
     * {@link com.marshalchen.ultimaterecyclerview.exchange.ExchangeLayout.ShowMode}.PullOut and
     * {@link com.marshalchen.ultimaterecyclerview.exchange.ExchangeLayout.ShowMode}.LayDown.
     *
     * @param mode na
     */
     ...
     * get the open status.
     *
     * @return {@link com.marshalchen.ultimaterecyclerview.exchange.ExchangeLayout.Status} Open , Close or
     * Middle.
     */
    ...
    public void open(boolean smooth, boolean notify) {
        ...
        if (smooth) {
            ...
            if (notify) {
                ...
                dispatchExchangeEvent(rect.left, rect.top, dx, dy);
            } else {
                safeBottomView();
            }
        }
    ...
    public interface DoubleClickListener {
        void onDoubleClick(ExchangeLayout layout, boolean surface);
    }
    ...
    private float getCurrentOffset() {
        ...
        return mEdgeExchangesOffset[mCurrentDragEdge.ordinal()];
    }
